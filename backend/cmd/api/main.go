...
func main() {
	...
	apiConnStr := fmt.Sprintf("host=%s port=%s dbname=%s user=%s password=%s sslmode=disable",
	...
	routerConnStr := fmt.Sprintf("host=%s port=%s dbname=%s user=router_ro password=%s sslmode=disable",
		dbHost, dbPort, dbName, getEnv("ROUTER_PASSWORD", ""))
	routerStore, err := store.NewRouterStore(routerConnStr)
	if err != nil {
	...
	r := router.NewRouter(quoteEngine, routerStore)
	...
	handlers := api.NewHandlers(r, apiStore, kvStore)
